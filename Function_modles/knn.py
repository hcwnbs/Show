...
def trainingDataSet():
    hwLabels = []
    trainingFileList = listdir('E:/Notes/python/Project/Django/Show/static/digits/trainingDigits')           #获取目录内容
    m = len(trainingFileList)
    trainingMat = zeros((m,1024))                          #m维向量的训练集
    for i in range(m):
        fileNameStr = trainingFileList[i]
        hwLabels.append(classnumCut(fileNameStr))
        trainingMat[i,:] = img2vector('E:/Notes/python/Project/Django/Show/static/digits/trainingDigits/%s' % fileNameStr)
    return hwLabels,trainingMat

#测试函数
def handwritingTest():
    hwLabels,trainingMat = trainingDataSet()    #构建训练集
    testFileList = listdir('E:/Notes/python/Project/Django/Show/static/digits/testDigits')        #获取测试集
    errorCount = 0.0                            #错误数
    ...
    for i in range(mTest):
        ...
        vectorUnderTest = img2vector('E:/Notes/python/Project/Django/Show/static/digits/testDigits/%s' % fileNameStr)
        #调用knn算法进行测试
...
def handwriting(filename):
    ...
    # 调用knn算法进行识别
    classifierResult = classify(vectorUnderTest, trainingMat, hwLabels, 2)
    #返回识别结果
